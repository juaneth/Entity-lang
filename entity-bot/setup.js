...
try {
    fs.mkdirSync('../bot-config');
    console.log(`Setup made folder: ../bot-config`);
} catch (err) {
    console.log(err + "\n");
}

try {
    fs.writeFileSync("../bot-config/config.json", JSON.stringify(defaultconfig, null, 4), 'utf-8');
    ...
    console.log('Writing file failed: ../bot-config/config.json');
}